...
import { sampleWithRequiredData } from '../academic-year.test-samples';
...
describe('AcademicYear Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AcademicYear;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 29518 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.academicYears()[0]).toEqual(expect.objectContaining({ id: 29518 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 29518 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.academicYears()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 29518 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to academicYearService', () => {
      ...
      vitest.spyOn(service, 'getAcademicYearIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAcademicYearIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.academicYearsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
