...
import { sampleWithRequiredData } from '../academic-terms.test-samples';
...
describe('AcademicTerms Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AcademicTerms;
  let fixture: ComponentFixture<AcademicTerms>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AcademicTerms);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 24556 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.academicTermses()[0]).toEqual(expect.objectContaining({ id: 24556 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 24556 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.academicTermses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 24556 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to academicTermsService', () => {
      ...
      vitest.spyOn(service, 'getAcademicTermsIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAcademicTermsIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.academicTermsesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
