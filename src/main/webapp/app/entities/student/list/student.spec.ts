...
describe('Student Management Component', () => {
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 9978 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    successReq.flush([{ id: 9978 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to studentService', () => {
      ...
      vitest.spyOn(service, 'getStudentIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.studentsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
