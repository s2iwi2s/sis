...
import gradeLevelPayablesResolve from './grade-level-payables-routing-resolve.service';

describe('GradeLevelPayables routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IGradeLevelPayables returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          gradeLevelPayablesResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(gradeLevelPayablesResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
