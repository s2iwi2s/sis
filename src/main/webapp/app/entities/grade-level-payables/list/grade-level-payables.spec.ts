...
import { sampleWithRequiredData } from '../grade-level-payables.test-samples';
import { GradeLevelPayablesService } from '../service/grade-level-payables.service';

import { GradeLevelPayables } from './grade-level-payables';

vitest.useFakeTimers();

describe('GradeLevelPayables Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: GradeLevelPayables;
  let fixture: ComponentFixture<GradeLevelPayables>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(GradeLevelPayables);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 14057 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.gradeLevelPayableses()[0]).toEqual(expect.objectContaining({ id: 14057 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 14057 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.gradeLevelPayableses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 14057 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to gradeLevelPayablesService', () => {
      ...
      vitest.spyOn(service, 'getGradeLevelPayablesIdentifier');
      const id = comp.trackId(entity);
      expect(service.getGradeLevelPayablesIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.gradeLevelPayablesesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
