...
import { sampleWithRequiredData } from '../learning-competency.test-samples';
import { LearningCompetencyService } from '../service/learning-competency.service';

import { LearningCompetency } from './learning-competency';

vitest.useFakeTimers();

describe('LearningCompetency Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: LearningCompetency;
  let fixture: ComponentFixture<LearningCompetency>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(LearningCompetency);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 10670 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.learningCompetencies()[0]).toEqual(expect.objectContaining({ id: 10670 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 10670 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.learningCompetencies()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 10670 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to learningCompetencyService', () => {
      ...
      vitest.spyOn(service, 'getLearningCompetencyIdentifier');
      const id = comp.trackId(entity);
      expect(service.getLearningCompetencyIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.learningCompetenciesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
