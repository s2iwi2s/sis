...
import { sampleWithRequiredData } from '../curriculum-map.test-samples';
...
describe('CurriculumMap Management Component', () => {
  ...
  let comp: CurriculumMap;
  let fixture: ComponentFixture<CurriculumMap>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CurriculumMap);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 22840 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.curriculumMaps()[0]).toEqual(expect.objectContaining({ id: 22840 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 22840 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.curriculumMaps()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 22840 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to curriculumMapService', () => {
      ...
      vitest.spyOn(service, 'getCurriculumMapIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.curriculumMapsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
