...
import { sampleWithRequiredData } from '../resources.test-samples';
...
describe('Resources Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 3547 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.resourceses()[0]).toEqual(expect.objectContaining({ id: 3547 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 3547 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.resourceses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 3547 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to resourcesService', () => {
      ...
      vitest.spyOn(service, 'getResourcesIdentifier');
      const id = comp.trackId(entity);
      expect(service.getResourcesIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.resourcesesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
