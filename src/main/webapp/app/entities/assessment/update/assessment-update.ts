  ...
  private deleteResourceFromForm(resourceToRemove: IResources): void {
    if (this.assessment) {
      if (!this.assessment.resourceses) {
        this.assessment.resourceses = [];
      }
      this.assessment.resourceses = this.assessment.resourceses.filter(r => r.id !== resourceToRemove.id);
      this.editForm.patchValue({
        resourceses: this.assessment.resourceses,
      });
    }
  }
        ...
        switchMap(async () => {
          modalRef.componentInstance.save(
            (resourcesAry: Pick<IResources, 'id' | 'fileName' | 'documentContentType'>[], activeModal: NgbActiveModal) => {
              if (this.assessment) {
                this.assessment.resourceses = [...(this.assessment.resourceses ? this.assessment.resourceses : []), ...resourcesAry];
