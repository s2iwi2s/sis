...
import { sampleWithRequiredData } from '../course-schedule.test-samples';
...
describe('CourseSchedule Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CourseSchedule;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 3926 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.courseSchedules()[0]).toEqual(expect.objectContaining({ id: 3926 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 3926 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.courseSchedules()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 3926 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to courseScheduleService', () => {
      ...
      vitest.spyOn(service, 'getCourseScheduleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCourseScheduleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.courseSchedulesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
