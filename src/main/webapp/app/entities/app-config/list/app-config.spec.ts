...
import { sampleWithRequiredData } from '../app-config.test-samples';
...
describe('AppConfig Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 10896 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.appConfigs()[0]).toEqual(expect.objectContaining({ id: 10896 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 10896 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    successReq.flush([{ id: 10896 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to appConfigService', () => {
      ...
      vitest.spyOn(service, 'getAppConfigIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.appConfigsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
