...
import { sampleWithRequiredData } from '../payments.test-samples';
...
describe('Payments Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 15423 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.paymentses()[0]).toEqual(expect.objectContaining({ id: 15423 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 15423 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.paymentses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 15423 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to paymentsService', () => {
      const entity = { id: 15423 };
      vitest.spyOn(service, 'getPaymentsIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.paymentsesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
