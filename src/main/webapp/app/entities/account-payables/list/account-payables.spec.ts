...
import { sampleWithRequiredData } from '../account-payables.test-samples';
...
describe('AccountPayables Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: AccountPayables;
  let fixture: ComponentFixture<AccountPayables>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(AccountPayables);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 21189 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.accountPayableses()[0]).toEqual(expect.objectContaining({ id: 21189 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 21189 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.accountPayableses()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 21189 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to accountPayablesService', () => {
      const entity = { id: 21189 };
      vitest.spyOn(service, 'getAccountPayablesIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAccountPayablesIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.accountPayablesesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
