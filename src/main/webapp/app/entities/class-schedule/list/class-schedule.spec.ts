...
import { sampleWithRequiredData } from '../class-schedule.test-samples';
...
describe('ClassSchedule Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: ClassSchedule;
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 13254 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.classSchedules()[0]).toEqual(expect.objectContaining({ id: 13254 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 13254 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.classSchedules()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 13254 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to classScheduleService', () => {
      ...
      vitest.spyOn(service, 'getClassScheduleIdentifier');
      const id = comp.trackId(entity);
      expect(service.getClassScheduleIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.classSchedulesParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
