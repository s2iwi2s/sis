...
public class InstructorRepositoryWithBagRelationshipsImpl implements InstructorRepositoryWithBagRelationships {
    ...
    private static final String INSTRUCTORS_PARAMETER = "instructors";
    ...
    public Optional<Instructor> fetchBagRelationships(Optional<Instructor> instructor) {
        return instructor.map(this::fetchCourseSchedules);
    }
    ...
    public List<Instructor> fetchBagRelationships(List<Instructor> instructors) {
        return Optional.of(instructors).map(this::fetchCourseSchedules).orElse(List.of());
    }

    Instructor fetchCourseSchedules(Instructor result) {
        return entityManager
            .createQuery(
                "select instructor from Instructor instructor left join fetch instructor.courseSchedules where instructor.id = :id",
                Instructor.class
            )
    ...
    List<Instructor> fetchCourseSchedules(List<Instructor> instructors) {
        ...
        List<Instructor> result = entityManager
            .createQuery(
                "select instructor from Instructor instructor left join fetch instructor.courseSchedules where instructor in :instructors",
                Instructor.class
            )
            .setParameter(INSTRUCTORS_PARAMETER, instructors)
            .getResultList();
